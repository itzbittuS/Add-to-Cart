...
let getPro = JSON.parse(localStorage.getItem("products")) || [];

let disPlay = () => {
    let str = '';
    getPro.forEach((prod) => {
        str += `<div class="pcard col-xl-2 col-lg-4 col-md-4  col-sm-12  mt-2 mx-2 position-relative d-flex flex-column px-0">
                <img src="${prod.thumbnail}" alt="" class="imgcard">
                <h3 class="mt-3 p-3">${prod.title}</h3>
                <h4 class="px-3 ">₹${prod.price}</h4>
                <button class=" b1 deletebtn" onclick="return cartcAdd(${prod.id}), updateBadge()">Add Cart</button>
                <button class=" b2 deletebtn">Buy Now</button>
            </div>`
    });
...
let cartcAdd = (id) => {
    ...
    let findProduct = cpro.find((get) => get.id == id);
    let update = myCart.find((ele) => ele.id == findProduct.id);

    if (myCart.find((ele) => ele.id == id)) {
        if (update) {
            update.qty = update.qty + 1;
            update.Price = update.qty * update.unitPrice;
        }
    } else {
        let data = {
            id: findProduct.id,
...
let updateBadge = () => {
    document.querySelector('.badge').innerHTML = myCart.length;
}
